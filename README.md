...
> [!WARNING]
> **このワークフローの品質は実用レベルにありません**
> walk / run / idle のようなロコモーションは実用レベルですが、
> aim, jump, roll など一般的でない姿勢のモーションは SD1.5 + AnimateDiff + キャラ LoRA では品質が出ません(姿勢が塊になるなど)
> 画像生成はキャラクターシート・デザインのみにし、 Tripo で 3D 生成、アニメーションリターゲットし、それを2Dスプライトにする方面を検討中
> このリポジトリはその判断に至るまでの実験と、流用できる後段処理（`pixelate` / `sheet`）です。
