...
> [!WARNING]
> **このリポジトリの開発は停止しています。**
> 試した結果、walk / run / idle のようなロコモーションは実用になりましたが、
> 剣を振る・前転する・弓を構えるといった小道具付き・希少姿勢のモーションは
> SD1.5 + AnimateDiff + キャラ LoRA では品質が出ませんでした（小道具が描かれない、姿勢が塊になる）。
> 以降は、SD で作ったキャラクターデザインシートから Tripo AI で 3D モデルを生成し、
> Mixamo / Quaternius UAL のアニメーションをリターゲットして 8 方向にレンダリング + ドット化する
> （または 3D モデルのまま 3D ゲームにする）方向に切り替えます。
> ここに残っているのは、その判断に至るまでの実験と、流用できる後段処理（`pixelate` / `sheet`）です。
